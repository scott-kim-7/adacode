...
def test_toolsandbox_smoke_script(require_stack):
	script = _repo_root() / "scripts" / "eval" / "run-toolsandbox-smoke.sh"
	result = subprocess.run([str(script)], capture_output=True, text=True, check=False)
	assert result.returncode == 0, result.stderr
	out = results_dir() / "toolsandbox-smoke.json"
	payload = load_json_file(out)
	validate_result_schema(payload)
	ok, msg = compare_baseline("toolsandbox", float(payload["pass_rate"]))
	assert ok, msg
