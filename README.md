...
# 2) 로컬 LLM — mlx_lm / mlx-vlm을 :8089에서 **별도로** 기동 (Ada가 관리하지 않음)
...
```
Browser → Open WebUI (:3000) → LangGraph agent (:9082) → MLX OpenAPI (:8089)
```
...
| `ADA_MLX_PORT` | `8089` | MLX API 포트 (호스트 `8080–8090`은 MLX 전용) |
